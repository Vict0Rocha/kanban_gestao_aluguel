...
erDiagram
    ...
    PARCELA_LANCAMENTOS |o--o{ TAXAS_IMOBILIARIA : "cascade (Fase 14)"
...
- **taxas_imobiliaria** (Phase 13) — o livro-razão paralelo da taxa que a própria imobiliária recebe — administração ou comissão do primeiro aluguel, sempre ligado a uma parcela e a um contrato, gerado no momento da baixa com tipo e valor livres — o diálogo só pré-seleciona o tipo (comissão do primeiro aluguel na parcela de menor competência do contrato, administração nas demais) e sugere o valor, e o usuário pode trocar os dois, por exemplo para cobrar a comissão só no segundo mês; ver [fronteira estrutural entre taxa e livro-razão do aluguel](#decisões-de-design). `lancamento_id` (Phase 14, nullable, `on delete cascade`) liga a taxa ao lançamento de pagamento específico que a gerou — só para limpeza automática quando esse pagamento é cancelado, nunca para cálculo; linhas de taxa geradas antes da Phase 14 ficam com `lancamento_id` nulo para sempre (sem backfill).
...
- **column_id nullable ao arquivar (D-01/D-02, `16-CONTEXT.md`)** — `cards.column_id` era `not null` desde o schema inicial; a Phase 16 relaxou essa constraint (`alter table cards alter column column_id drop not null`) e passou a gravar `column_id = null` sempre que um card é arquivado (`arquivarCardAction`) — antes, o card continuava apontando para a coluna em que estava, mesmo arquivado. Desarquivar (`desarquivarCardAction`) nunca devolve essa coluna antiga: sempre atribui a primeira coluna do board (menor `position`), resolvida pela mesma consulta de `boards` (`order by created_at`, `limit 1`) que `page.tsx` já usa para achar "o" board — este projeto tem um único board, então não existe ambiguidade. Um board sem nenhuma coluna bloqueia o desarquivamento com mensagem clara, em vez de gravar um card `arquivado_em = null` sem nenhuma coluna (que ficaria invisível em toda tela). O porquê: antes desta fase, um card arquivado com `column_id` ainda apontando para uma coluna real era apagado em cascata (`on delete cascade` de `columns`) se essa coluna fosse excluída — sem nenhum aviso, porque o trigger `cards_impede_exclusao_com_lancamento` (bullet seguinte) só bloqueia quando existe lançamento financeiro de verdade, e um card arquivado sem histórico nenhum não bloqueia nada. Um `column_id` nulo nunca é alcançado por `on delete cascade` — o risco fecha por construção, não por uma checagem que o código da aplicação precisa lembrar de fazer. A mesma migração inclui um backfill único (`update cards set column_id = null where arquivado_em is not null`) para cobrir também os cards já arquivados antes desta fase.
- **O trigger `cards_impede_exclusao_com_lancamento` como backstop de banco contra exclusão com histórico financeiro** — `cards → parcelas → parcela_lancamentos` é `on delete cascade` desde `20260816000000_financeiro_schema.sql`, e `columns → cards` é `on delete cascade` desde o schema inicial (`20260728000000_init_schema.sql`) — ou seja, existem **dois** caminhos que apagam histórico financeiro em cascata, e a Server Action de exclusão de card (`deleteCardAction`) só cobre um deles (o outro, excluir a coluna inteira, nunca foi coberto **até a Phase 17** — ver bullet "Coluna sempre vazia antes de excluir" logo abaixo, que fecha esse segundo caminho na camada de aplicação). O trigger `before delete on cards for each row` cobre os dois e sobrevive a um caminho de código futuro que esqueça a trava — exatamente como o buraco atual nasceu (`deleteCardAction` foi escrita antes de `parcelas` existir e nunca foi revisitada). A trava dispara para `parcela_lancamentos` ligado ao card com `tipo` em `pagamento`/`acréscimo`/`desconto` — a Phase 15 (D-01/D-03, `15-CONTEXT.md`) relaxou pontualmente esse predicado para excluir `tipo='destrava'`: um lançamento de destrava nunca soma valor (é registro de auditoria de que uma parcela foi destravada, não dinheiro se movendo — mesma razão que já o excluía do cancelamento em `cancelarLancamentoAction`, D-01 `12-CONTEXT.md`, também reaberto pela Phase 15), então um card cujo único histórico financeiro é `destrava` pode ser excluído de verdade. `taxas_imobiliaria`/`caucao_eventos` continuam bloqueando por qualquer linha, sem exceção de tipo — as duas tabelas não têm coluna `tipo` equivalente, cada linha já É dinheiro de verdade. O trigger roda `security invoker` (não `definer`): com `definer`, o `select` interno ignoraria o RLS de `parcelas`/`parcela_lancamentos`, vazando "existe lançamento" para quem nem deveria ver o card. Deliberadamente **não** virou `on delete restrict` na FK: isso bloquearia também a exclusão de um contrato que só tem parcelas geradas automaticamente, sem nenhum lançamento — o que a regra de negócio quer permitir — e seria alterar uma FK existente em vez de só acrescentar algo novo.
- **Coluna sempre vazia antes de excluir, sem migração nova (D-01/D-02/D-03, Phase 17)** — o FK `on delete cascade` de `columns → cards` (bullet "Cascata" acima) permanece exatamente como está no schema, como rede de segurança; nenhuma migração foi criada nesta fase — a primeira desde a Phase 4 sem nenhum arquivo novo em `supabase/migrations/`. A partir desta fase, a aplicação garante que a coluna tem zero cards antes do `delete` rodar: `excluirColunaComMovimentoAction` (`web/src/lib/kanban/actions.ts`) reconsulta os cards da coluna de origem e o `board_id` da coluna de destino no momento da escrita — nunca confia numa lista vinda do cliente — e move todos os cards para a coluna escolhida antes de excluir a coluna de origem, numa única chamada de servidor. Se não existe nenhuma outra coluna no board, a exclusão é bloqueada com mensagem clara em vez de deixar a coluna sem para onde mover os cards (D-02). `deleteColumnAction` também passou a recusar sozinha excluir uma coluna não vazia, mesmo se chamada fora da interface — sem essa trava própria, a garantia de EXCOL-04 valeria só por convenção de UI, não no boundary real do servidor onde Server Actions são endpoints POST de verdade.
...
- **Caução é histórico, não coluna (D-06, Phase 13)** — `caucao_eventos` não tem coluna de saldo; o saldo de um contrato é sempre a soma dos eventos (`recebido − devolvido − usado`), mesmo princípio já usado para `valorDevido`/`valorPago` de uma parcela. Cada evento é uma linha nova, nunca um `UPDATE` — não existe (e não deve existir) forma de editar um evento já gravado. (Cancelamento sequencial a partir do evento mais recente foi acrescentado na Phase 14 — ver [Cancelamento de taxa e de caução](#decisões-de-design) — sem contradizer o parágrafo acima: cada evento continua nascendo como linha nova, nunca editado; o que muda é a possibilidade de apagar o topo atual.)
- **`taxas_imobiliaria.lancamento_id` reabre D-04 só para cascata (D-03, Phase 14)** — a fronteira estrutural acima continua valendo por inteiro para cálculo de valor/status; a coluna nova existe com um propósito único e estreito: quando um pagamento (`parcela_lancamentos`) é cancelado (Phase 11/12), o `on delete cascade` desta FK remove automaticamente a taxa que aquele pagamento específico gerou, sem nenhum código de aplicação novo em `cancelarLancamentoAction` — a cascata é 100% do banco. Nullable e sem backfill: taxas geradas antes da Phase 14 (inclusive as de teste da Phase 13) ficam com `lancamento_id` nulo para sempre, porque não há como inferir retroativamente qual pagamento gerou qual taxa quando uma parcela paga em partes acumulou mais de uma. O mecanismo de cancelamento independente da taxa (sem cascata) e o cancelamento sequencial de caução chegam no restante da Phase 14 — ver "Cancelamento de taxa e de caução" logo abaixo.
- **Cancelamento de taxa e de caução (D-02/D-04/D-05, Phase 14)** — três operações novas, todas reusando o mesmo diálogo de confirmação simples (`CancelarLancamentoDialog`, generalizado por uma prop `rotulo: string` e `acao: "lancamento" | "taxa" | "caucao"` em vez de um union de tipo específico), sem motivo obrigatório, DELETE real, mesmo padrão já estabelecido em Phase 11/12: (1) uma taxa da imobiliária ganha seu próprio botão "Cancelar" no histórico da parcela (`cancelarTaxaImobiliariaAction`), com a mesma trava de parcela conciliada e sem nunca chamar `recalcularEGravarStatus` — cancelar uma taxa nunca mexe no valor devido/pago/status da parcela; (2) cancelar o pagamento que gerou aquela taxa (Phase 11/12) agora também remove a taxa automaticamente, via `on delete cascade` na FK `lancamento_id` (ver bullet acima) — nenhum código novo em `cancelarLancamentoAction`; (3) o histórico de caução (`caucao_eventos`, append-only desde a Phase 13, D-06) ganha `cancelarEventoCaucaoAction`, mas só sobre o evento mais recente por vez — o servidor relê `order by criado_em desc limit 1` e recusa se o `id` recebido não bater com o mais recente, mesmo padrão de checagem em duas etapas que `destravarParcelaAction` já usa. Cancelar o mais recente libera o que ficou no novo topo, permitindo desfazer o ciclo inteiro, evento por evento, sempre a partir do topo — nunca um evento do meio.
