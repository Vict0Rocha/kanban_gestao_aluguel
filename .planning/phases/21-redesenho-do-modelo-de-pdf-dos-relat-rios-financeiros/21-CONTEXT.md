...
O projeto tem dois PDFs exportáveis, ambos gerados client-side com `jsPDF`+`jspdf-autotable`: o Relatório Financeiro dedicado (`relatorio-financeiro-pdf.ts`, Phase 10) e o Dinheiro da imobiliária (`reconciliacao-pdf.ts`, Phase 19). Os dois seguem hoje o mesmo "PDF Export Layout Contract" (`10-UI-SPEC.md`): A4 retrato, paleta verde clara (`#18341c`/`#5c7060`/`#dbeed4`/`#74ac1c`/`#eaf6e6`), sem linha de total. Esta fase substitui esse contrato por um novo, aplicado aos dois PDFs igualmente (mesma orientação/paleta/estilo de tabela/linha de total) e nos três blocos estruturais que já existem (título, filtros aplicados, totais/resumo) — só o visual desses blocos muda, não o conteúdo. Uma exceção pontual: as colunas de `reconciliacao-pdf.ts` (Dinheiro da imobiliária) **mudam nesta fase** — ganham a coluna Inquilino e trocam endereço por proprietário na coluna Contrato, fechando uma mudança que a Phase 20 deixou deliberadamente pendente (D-02, corrigido após pesquisa). `relatorio-financeiro-pdf.ts` mantém suas colunas atuais.
...
- **D-02:** O redesenho vale para **os dois PDFs** — Relatório Financeiro dedicado e Dinheiro da imobiliária — com o mesmo visual novo (paleta, estilo de linha, cabeçalho/Total).
  **Correção pós-pesquisa (21-RESEARCH.md):** ao contrário do que esta decisão dizia originalmente, `reconciliacao-pdf.ts` **não** foi atualizado na Phase 20 — a Phase 20 trocou as colunas só na tela (`dinheiro-imobiliaria-view.tsx`), deixando o PDF deliberadamente intocado (D-08/D-09 da própria `20-CONTEXT.md`, confirmado por leitura direta do código: `head: [["Data", "Contrato", "Tipo", "Valor", "Observação"]]`, "Contrato" ainda mostra `#numero endereco`, sem coluna Inquilino). O pedido do usuário nesta mensagem — "colunas... data, contrato com o id e nome do proprietário, inquilino, tipo, valor e observação" — é exatamente a atualização que ficou pendente. Decisão corrigida: **`reconciliacao-pdf.ts` GANHA a mudança de colunas nesta fase** (Contrato troca endereço por proprietário, mesmo formato "#numero nome"; nova coluna Inquilino) — fechando o D-09 que a Phase 20 deixou em aberto. `relatorio-financeiro-pdf.ts` continua com suas colunas atuais inalteradas (Imóvel/Proprietário/Competência/Vencimento/Situação/Valor) — o usuário nunca pediu mudança nelas.
